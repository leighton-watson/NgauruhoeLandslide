...
SUMMARY_CSV = HERE / "Outputs" / "benchmark_runtime_summary.csv"
SCALING_CSV = HERE / "Outputs" / "benchmark_station_scaling.csv"

TOTAL_REPEATS = 10
LOCALISATION_REPEATS = 10
...
def _build_asl_spec(stations):
    ...
    site_amplification_csv = (
        asl_cfg_mod.SITE_AMPLIFICATION_CSV
        if asl_cfg_mod.USE_SITE_AMPLIFICATION
        else None
    )
    attenuation_coefficient_per_km = (
        asl_cfg_mod.ATTENUATION_COEFFICIENT_PER_KM
        if asl_cfg_mod.USE_INTRINSIC_ATTENUATION
        else 0.0
    )
    cfg = dict(
        ...
        USE_SITE_AMPLIFICATION=asl_cfg_mod.USE_SITE_AMPLIFICATION,
        SITE_AMPLIFICATION_CSV=site_amplification_csv,
        SITE_AMPLIFICATION_COLUMN=asl_cfg_mod.SITE_AMPLIFICATION_COLUMN,
        USE_INTRINSIC_ATTENUATION=asl_cfg_mod.USE_INTRINSIC_ATTENUATION,
    ...
    load_kwargs = {
        ...
        "event": dict(
            ...
            site_amplification_csv=cfg["SITE_AMPLIFICATION_CSV"],
            site_amplification_column=cfg["SITE_AMPLIFICATION_COLUMN"],
        ),
    }
    localisation_kwargs = dict(
        ...
        b=cfg["B"],
        attenuation_coefficient_per_km=cfg["ATTENUATION_COEFFICIENT_PER_KM"],
...
def _build_sara_spec(stations):
    ...
    site_amplification_csv = (
        sara_cfg_mod.SITE_AMPLIFICATION_CSV
        if sara_cfg_mod.USE_SITE_AMPLIFICATION
        else None
    )
    attenuation_coefficient_per_km = (
        sara_cfg_mod.ATTENUATION_COEFFICIENT_PER_KM
        if sara_cfg_mod.USE_INTRINSIC_ATTENUATION
        else 0.0
    )
    cfg = dict(
        ...
        USE_SITE_AMPLIFICATION=sara_cfg_mod.USE_SITE_AMPLIFICATION,
        SITE_AMPLIFICATION_CSV=site_amplification_csv,
        SITE_AMPLIFICATION_COLUMN=sara_cfg_mod.SITE_AMPLIFICATION_COLUMN,
        USE_INTRINSIC_ATTENUATION=sara_cfg_mod.USE_INTRINSIC_ATTENUATION,
        ATTENUATION_COEFFICIENT_PER_KM=attenuation_coefficient_per_km,
...
def _benchmark_method(method_name, stations):
    ...
    summary = {
        ...
        "pairs": pair_count,
        "site_amplification_applied": bool(cfg["USE_SITE_AMPLIFICATION"]),
        "site_amplification_csv": str(cfg["SITE_AMPLIFICATION_CSV"] or ""),
        "intrinsic_attenuation_applied": bool(cfg["USE_INTRINSIC_ATTENUATION"]),
        "attenuation_coefficient_per_km": float(cfg["ATTENUATION_COEFFICIENT_PER_KM"]),
        "geometric_spreading_exponent": float(
            cfg["B"] if method_name == "ASL" else cfg["DECAY_EXPONENT"]
        ),
        "grid_nx": nx,
        "grid_ny": ny,
        "time_slices": n_time,
...
if __name__ == "__main__":
    ...
    print(
        "ASL corrections: "
        f"site amplification={'ENABLED' if asl_cfg_mod.USE_SITE_AMPLIFICATION else 'DISABLED'}, "
        f"intrinsic attenuation={'ENABLED' if asl_cfg_mod.USE_INTRINSIC_ATTENUATION else 'DISABLED'}, "
        f"B={asl_cfg_mod.ATTENUATION_COEFFICIENT_PER_KM:.8f} km^-1, "
        f"geometric b={asl_cfg_mod.B:g}"
    )
    print(
        "SARA corrections: "
        f"site amplification={'ENABLED' if sara_cfg_mod.USE_SITE_AMPLIFICATION else 'DISABLED'}, "
        f"intrinsic attenuation={'ENABLED' if sara_cfg_mod.USE_INTRINSIC_ATTENUATION else 'DISABLED'}, "
        f"B={sara_cfg_mod.ATTENUATION_COEFFICIENT_PER_KM:.8f} km^-1, "
        f"geometric b={sara_cfg_mod.DECAY_EXPONENT:g}"
    )
    print(
        "Benchmark overrides: "
