...
def load_site_amplification_factors(
    stations,
    freq_min,
    freq_max,
    summary_csv,
    factor_column="site_amplification",
    frequency_tolerance_hz=1e-6,
):
    """Return site-amplification factors ordered to match stations."""
    stations = [str(station).strip() for station in stations]
    ...
    if missing_columns:
        raise ValueError(
            f"{summary_path} is missing required columns: {sorted(missing_columns)}"
        )

    table["station"] = table["station"].astype(str).str.strip()
    band_mask = (
        np.isclose(table["frequency_min_hz"].astype(float), float(freq_min), atol=frequency_tolerance_hz)
        & np.isclose(table["frequency_max_hz"].astype(float), float(freq_max), atol=frequency_tolerance_hz)
    )
    band_table = table.loc[band_mask].copy()
    if band_table.empty:
        available_bands = sorted(
            {
                (float(row.frequency_min_hz), float(row.frequency_max_hz))
                for row in table[["frequency_min_hz", "frequency_max_hz"]].itertuples(index=False)
            }
        )
        raise ValueError(
            f"No site-amplification rows for {freq_min}-{freq_max} Hz in {summary_path}. "
            f"Available bands: {available_bands}"
        )

    if band_table["station"].duplicated().any():
        duplicates = sorted(band_table.loc[band_table["station"].duplicated(), "station"].unique())
        raise ValueError(
            f"Duplicate station rows for {freq_min}-{freq_max} Hz in {summary_path}: {duplicates}"
        )

    factor_lookup = {
        str(row.station): float(getattr(row, factor_column))
        for row in band_table[["station", factor_column]].itertuples(index=False)
    }
    missing_stations = [station for station in stations if station not in factor_lookup]
    if missing_stations:
        raise ValueError(
            f"Missing site-amplification factors for stations {missing_stations} "
            f"at {freq_min}-{freq_max} Hz in {summary_path}"
        )

    factors = np.asarray([factor_lookup[station] for station in stations], dtype=float)
    if np.any(~np.isfinite(factors)) or np.any(factors <= 0.0):
        bad = [
            (station, factor)
            for station, factor in zip(stations, factors)
            if not np.isfinite(factor) or factor <= 0.0
        ]
        raise ValueError(f"Site-amplification factors must be finite and positive: {bad}")

    return factors


def attenuation_coefficient_per_m(dominant_frequency_hz, quality_factor, shear_wave_speed_m_s):
    """Return B in 1/m for exp(-B r). Use Q=0 for no intrinsic attenuation."""
    frequency = float(dominant_frequency_hz)
    q_value = float(quality_factor)
    beta = float(shear_wave_speed_m_s)

    if frequency <= 0.0:
        raise ValueError("dominant_frequency_hz must be positive.")
    if beta <= 0.0:
        raise ValueError("shear_wave_speed_m_s must be positive.")
    if q_value < 0.0:
        raise ValueError("quality_factor must be non-negative; use 0 for no attenuation.")
    if q_value == 0.0 or np.isinf(q_value):
        return 0.0

    return float(np.pi * frequency / (q_value * beta))
...
def load_event_data(
    ...
    site_amplification = {
        "applied": False,
        "csv": None,
        "column": site_amplification_column,
        "factors": np.ones(len(stations), dtype=float),
    }
    if site_amplification_csv is not None:
        factors = load_site_amplification_factors(
            stations=stations,
            freq_min=freq_min,
            freq_max=freq_max,
            summary_csv=site_amplification_csv,
            factor_column=site_amplification_column,
        )
        for station_idx, factor in enumerate(factors):
            rms_data[station_idx] = rms_data[station_idx].copy()
            rms_data[station_idx][:, 1] = rms_data[station_idx][:, 1] / factor
        site_amplification = {
            "applied": True,
            "csv": str(site_amplification_csv),
            "column": site_amplification_column,
            "factors": factors,
        }
        print("\n  Applied site-amplification correction:")
        for station, factor in zip(stations, factors):
            print(f"    {station:8s}  RMS /= {factor:.6g}")
...
def run_localization(
    ...
    b=1.0,
    quality_factor=0.0,
    shear_wave_speed_m_s=1000.0,
    dominant_frequency_hz=5.0,
    attenuation_coefficient_per_km=None,
    ...
    if attenuation_coefficient_per_km is None:
        attenuation_b = attenuation_coefficient_per_m(
            dominant_frequency_hz=dominant_frequency_hz,
            quality_factor=quality_factor,
            shear_wave_speed_m_s=shear_wave_speed_m_s,
        )
        attenuation_source = "Q"
    else:
        attenuation_b = float(attenuation_coefficient_per_km) / 1000.0
        if attenuation_b < 0.0:
            raise ValueError("attenuation_coefficient_per_km must be non-negative.")
        attenuation_source = "fixed_B"
    print(f"  Decay b  : {b}")
    print(f"  Q        : {quality_factor}")
    print(f"  beta     : {shear_wave_speed_m_s:.1f} m/s")
    print(f"  f_dom    : {dominant_frequency_hz:.3f} Hz")
    print(f"  B source : {attenuation_source}")
    print(f"  B        : {attenuation_b:.6e} 1/m  ({attenuation_b * 1000.0:.4f} 1/km)\n")
    ...
    logdist_centered = logdist - mean_logdist[:, None]
    dist_centered = dist - mean_dist[:, None]
    attenuation_shape = b * logdist_centered + attenuation_b * dist_centered
    attenuation_ss = np.sum(attenuation_shape ** 2, axis=1)[:, None]
    ...
    cross = attenuation_shape @ y_centered.T
    sse = sst[None, :] + 2.0 * cross + attenuation_ss
    ...
    intercept_flat = (
        mean_log_amplitude[None, :]
        + b * mean_logdist[:, None]
        + attenuation_b * mean_dist[:, None]
    )
    ...
    results = {
        ...
        "attenuation_coefficient_per_m": attenuation_b,
        "attenuation_coefficient_per_km": attenuation_b * 1000.0,
...
def build_output_bundle(cfg, topo, data, results):
    ...
    station_coords = data["station_coords"]
    site_amplification = data.get("site_amplification", {})
    site_amplification_factors = np.asarray(
        site_amplification.get("factors", np.ones(len(station_coords))),
        dtype=float,
    )
    ...
    return {
        ...
        "quality_description": (
            "Coefficient of determination from the amplitude-decay fit at each time step; "
            "the fit includes intrinsic attenuation when an attenuation coefficient is non-zero."
        ),
        ...
        "config": cfg,
        "filter_params": data["filter_params"],
        "site_amplification_applied": bool(site_amplification.get("applied", False)),
        "site_amplification_csv": site_amplification.get("csv", None),
        "site_amplification_column": site_amplification.get("column", None),
        "site_amplification_factors": site_amplification_factors,
        "analysis_params": {
            "b": results["b"],
            "quality_factor": results["quality_factor"],
            "shear_wave_speed_m_s": results["shear_wave_speed_m_s"],
            "dominant_frequency_hz": results["dominant_frequency_hz"],
            "attenuation_source": results.get("attenuation_source", "Q"),
            "attenuation_coefficient_per_m": results["attenuation_coefficient_per_m"],
            "attenuation_coefficient_per_km": results["attenuation_coefficient_per_km"],
...
def run_analysis(cfg):
    ...
    data = load_event_data(
        ...
        site_amplification_csv=cfg.get("SITE_AMPLIFICATION_CSV", None),
        site_amplification_column=cfg.get("SITE_AMPLIFICATION_COLUMN", "site_amplification"),
    )
    ...
    results = run_localization(
        ...
        temporal_step=cfg.get("TEMPORAL_STEP", 1.0),
        b=cfg.get("B", 1.0),
        quality_factor=cfg.get("QUALITY_FACTOR", 0.0),
        shear_wave_speed_m_s=cfg.get("SHEAR_WAVE_SPEED_M_S", 1000.0),
        dominant_frequency_hz=cfg.get("DOMINANT_FREQUENCY_HZ", 5.0),
        attenuation_coefficient_per_km=cfg.get("ATTENUATION_COEFFICIENT_PER_KM", None),
