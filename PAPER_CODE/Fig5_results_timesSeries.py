...
SAVE_FIG = HERE / "FIGURES" / "Figure5.pdf"
...
if __name__ == "__main__":
    ...
    ax_asl_q = fig.add_subplot(gs[1, 0])
    ax_sara_q = fig.add_subplot(gs[1, 1])
    ax_east = fig.add_subplot(gs[2, 0])
    ax_elev = fig.add_subplot(gs[3, 0])
    ax_north = fig.add_subplot(gs[2:4, 1])
    ...
    ax_ratio.set_title(f"(b) Station ratios", fontsize=TITLE_FONTSIZE, fontweight="bold")
    ...
    asl_time, asl_x = _crop_time_series(
    ...
    sara_time, sara_x = _crop_time_series(
    ...
    asl_crater_x = float(np.asarray(asl["crater_easting"], dtype=float)[0])
    asl_crater_y = float(np.asarray(asl["crater_northing"], dtype=float)[0])
    sara_crater_x = float(np.asarray(sara["crater_easting"], dtype=float)[0])
    sara_crater_y = float(np.asarray(sara["crater_northing"], dtype=float)[0])
    asl_east = asl_x - asl_crater_x
    asl_north = asl_y - asl_crater_y
    sara_east = sara_x - sara_crater_x
    sara_north = sara_y - sara_crater_y
    ...
    ax_east.set_ylabel("Easting from summit crater (m)", fontsize=AXIS_LABEL_FONTSIZE)
    ...
    ax_north.set_ylabel("Northing from summit crater (m)", fontsize=AXIS_LABEL_FONTSIZE)
    ...
    ax_north.legend(loc="best", fontsize=LEGEND_FONTSIZE)
