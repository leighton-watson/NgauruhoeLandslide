...
from asl import attenuation_coefficient_per_m, load_site_amplification_factors
from io_utils import get_plot_window_seconds, save_results_bundle, subset_topography
...
def load_event_data(
    ...
    site_amplification_band_hz=None,
):
    ...
    site_amplification_freq_min = freq_min
    site_amplification_freq_max = freq_max
    if site_amplification_band_hz is not None:
        site_amplification_freq_min, site_amplification_freq_max = site_amplification_band_hz
    ...
    site_amplification = {
        ...
        "frequency_min_hz": float(site_amplification_freq_min),
        "frequency_max_hz": float(site_amplification_freq_max),
    }
    if site_amplification_csv is not None:
        factors = load_site_amplification_factors(
            stations=stations,
            freq_min=site_amplification_freq_min,
            freq_max=site_amplification_freq_max,
            summary_csv=site_amplification_csv,
            factor_column=site_amplification_column,
        )
        rms = rms / factors[:, None]
        site_amplification = {
            "applied": True,
...
def amplitude_ratio_localization(
    ...
    logdist = np.log(dist)
    predicted_pairs = -(
        decay_exponent * (logdist[:, pair_idx[:, 0]] - logdist[:, pair_idx[:, 1]])
        + attenuation_b * (dist[:, pair_idx[:, 0]] - dist[:, pair_idx[:, 1]])
    )
...
def hybrid_localization(
    ...
    ratio_result = amplitude_ratio_localization(
        ...
        shear_wave_speed_m_s=shear_wave_speed_m_s,
        dominant_frequency_hz=dominant_frequency_hz,
        attenuation_coefficient_per_km=attenuation_coefficient_per_km,
    )
    ...
    return {
        ...
        "metadata": {
            ...
            "shear_wave_speed_m_s": shear_wave_speed_m_s,
            "dominant_frequency_hz": dominant_frequency_hz,
            "attenuation_source": ratio_result["metadata"].get("attenuation_source", "Q"),
            "attenuation_coefficient_per_m": ratio_result["metadata"]["attenuation_coefficient_per_m"],
            "attenuation_coefficient_per_km": ratio_result["metadata"]["attenuation_coefficient_per_km"],
            "velocity": velocity,
...
def build_output_bundle(cfg, topo, data, result):
    ...
    bundle = {
        ...
        "site_amplification_frequency_min_hz": site_amplification.get("frequency_min_hz", None),
        "site_amplification_frequency_max_hz": site_amplification.get("frequency_max_hz", None),
...
def run_method(method, cfg):
    ...
    data = load_event_data(
        ...
        site_amplification_band_hz=cfg.get("SITE_AMPLIFICATION_BAND_HZ", None),
    )
